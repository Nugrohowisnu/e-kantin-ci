...
class App extends CI_Controller {
	...
	public function hapus_semua_santri()
	{
		if ($this->session->userdata('level') == "") {
           redirect('app/login');
        }
	...
	public function simpan_penjualan()
	{
        ...
        $kode_penjualan = $this->input->post('kode_penjualan');
				if ($this->input->post('no_santri2')=='') {
					$no_santri = $this->input->post('no_santri');
	      }else {
					$no_santri = $this->input->post('no_santri2');
	      }
        $total_harga = $this->input->post('total_harga');
        $tgl_penjualan = $this->input->post('tgl_penjualan');
        $kasir = $this->input->post('kasir');

			if ($this->db->get_where('santri',array('no_santri'=>$no_santri))->num_rows()==0) {
				?>
					<script type="text/javascript">
						alert('NO. SISWA "<?php echo $no_santri; ?>" belum terdaftar disistem !');
						window.location="<?php echo base_url('app/tambah_penjualan'); ?>";
					</script>
				<?php exit;
			}elseif($total_harga=='' || $total_harga=='0'){
				redirect('app/tambah_penjualan');
			}else{
